...
def bfs(graph,source):
    ...
    traversal[idx] = source
    visitedList.append(source)
    bfsVisited.append(source)

    while len(visitedList) > 0:
        print visitedList

        currentNode = visitedList.pop(0)
        idx = getIndex(currentNode)
        for adjNodeIdx in range(len(y[idx])):
            adjNode = y[idx][adjNodeIdx][1]
            if(visited(adjNode) == False):
                trackVisited[getIndex(adjNode)] = VISITED
                visitedList.append(adjNode)
                bfsVisited.append(adjNode)
